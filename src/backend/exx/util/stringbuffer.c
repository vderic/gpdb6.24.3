...
struct StringBuffer *stringbuffer_new_with_options(const size_t initial_size, const bool allow_resize) {
	...
	struct StringBuffer *buffer = palloc(sizeof(struct StringBuffer));
	...
	buffer->work_buffer = palloc(STRINGBUFFER_WORK_BUFFER_SIZE * sizeof(char));
...
char *stringbuffer_to_string(struct StringBuffer *buffer) {
	if (buffer == NULL || buffer->content_size == 0) {
		char *string_copy = palloc(sizeof(char));
		if (string_copy == NULL) {
			return (NULL);
		}
		string_copy[0] = 0;
	...
	char *string_copy = palloc(memory_size);
	if (string_copy == NULL) {
		return (NULL);
	}
...
static bool _stringbuffer_clear(struct StringBuffer *buffer) {
	...
	buffer->value = palloc((buffer->max_size + 1) * sizeof(char));
	if (buffer->value == NULL) {
		stringbuffer_release(buffer);
		return (false);
	}
...
static bool _stringbuffer_set_capacity(struct StringBuffer *buffer, const size_t size) {
	...
	buffer->value = repalloc(buffer->value, (buffer->max_size + 1) * sizeof(char));
